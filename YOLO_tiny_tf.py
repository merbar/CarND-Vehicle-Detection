...
class YOLO_TF:
	...
	def detect_from_cvmat(self,imgs):
		s = time.time()
		...
		for img in imgs:
			...
			if not self.returnImg:
				img_RGB = cv2.cvtColor(img_resized,cv2.COLOR_BGR2RGB)
			else:
				img_RGB = np.copy(img_resized)
			img_resized_np = np.asarray( img_RGB )
			inputs.append = (img_resized_np/255.0)*2.0-1.0
	...
	def interpret_output(self,output):
		if self.returnData:
			for out_elem in output:
				...
				class_probs = np.reshape(out_elem[0:980],(7,7,20))
				scales = np.reshape(out_elem[980:1078],(7,7,2))
				boxes = np.reshape(out_elem[1078:],(7,7,2,4))
	...
	def show_results(self,img,results):
		...
		for i in range(len(results)):
			...
			if self.filewrite_img or self.imshow or self.returnImg:
