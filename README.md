...
- A search on the entire image using a large neural net would be very interesting to look into more. I tried a pre-trained YOLO-tiny with some success, and it runs very fast (20fps+). However, my computer's GPU RAM was insufficient to run the entire inference which lead to memory warnings and degraded accuracy.
